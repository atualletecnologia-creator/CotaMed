  ...
  async function processarPlanilha(file: File | null) {
    try {
      ...
      for (let index = 0; index < linhasNormalizadas.length; index++) {
        ...
        itensCotados.push(
          montarItemCotado({ index, descricao, quantidade, unidade, produto, margem: margemNumero, confianca: score, origemMatch: origem })
        );
      }

      setItens(itensCotados);
      setMensagem(`${itensCotados.length} itens processados. Use "Selecionar produto" quando o sistema não encontrar ou escolher errado.`);
    } finally {
    ...
    <AppShell>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Resultado em lista compacta, com seleção manual do produto quando necessário.
          </p>
        </div>
      ...
      <section className="card p-6 mt-6">
        ...
        <div className="bg-blue-50 rounded-2xl p-4 mt-5 text-sm text-slate-700">
          Se o sistema misturar ou não encontrar algum item, escolha o produto correto na lista de cada linha.
        </div>
        ...
        <>
          <section className="grid md:grid-cols-6 gap-4 mt-6">
            <div className="card p-4">
              ...
              <h3 className="text-xl font-bold text-green-700">{resumo.encontrados}</h3>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">Manual</p>
              <h3 className="text-xl font-bold text-blue-700">{resumo.manual}</h3>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">Conferir</p>
              <h3 className="text-xl font-bold text-yellow-700">{resumo.conferir}</h3>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">Não encontrados</p>
              <h3 className="text-xl font-bold text-red-700">{resumo.naoEncontrados}</h3>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">Excluídos</p>
              <h3 className="text-xl font-bold text-slate-700">{resumo.excluidos}</h3>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">Valor confirmado</p>
              <h3 className="text-xl font-bold">{dinheiro(resumo.total)}</h3>
            </div>
          </section>
                  ...
                  <div key={item.numero_item} className={item.excluido ? "rounded-xl border bg-slate-50 opacity-70 p-3" : "rounded-xl border bg-white p-3"}>
                    <div className="flex flex-wrap gap-3 text-[11px] leading-4">
                      <div className="w-12 shrink-0">
                        <Campo label="Item" value={<b>{item.numero_item}</b>} />
                      </div>
                      ...
                      <div className="w-12">
                        <Campo label="PDF" value={<span className={item.pdf_url && cotar ? "text-green-700" : "text-red-700"}>{item.pdf_url && cotar ? "Sim" : "Não"}</span>} />
                      </div>
